$(document).ready(function(){
	...
	for (let index = 0; index < actores.length; index++) {
		if(index<3)
            ...
            castContent += '<div class="actor hiddenActor">'
        castContent +=
			'<div class="actorInformation">'+
				...
				'<span> Age: ' + (2019 - actores[index].añoNac) + '</span>' +
			'</div>'+
			'</div>';
	}
	castContent += '</div><div id="view-more">Show more</div>';
	$('#cast').html(castContent);

    $('#view-more').click(function(){
        $('.hiddenActor').slideToggle(500);
        if($('#view-more').html() === 'Show more'){
            $('#view-more').html('Show less');
        }else{
            $('#view-more').html('Show more');
        }
    });
